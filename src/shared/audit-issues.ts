...
export const AUDIT_ISSUE_TYPES = {
  "blocked-page": {
    ...
    howToFix:
      'If you own this site, allowlist the "Sightline-Audit" user agent in your WAF/bot-protection settings (on Cloudflare: a WAF custom rule that skips bot protection when the user agent contains "Sightline-Audit"; on some free tiers you may need to relax bot protection). Then re-run the audit.',
  },
  "rate-limited-page": {
    severity: "warning",
    ...
    howToFix:
      'Raise the rate limit for crawlers, or allowlist the "Sightline-Audit" user agent in your rate-limiting rules (on Cloudflare: a rate-limiting rule exception matching that user agent). Then re-run the audit. Re-running with fewer pages also helps if the limit is strict.',
  },
  "crawl-rate-limited": {
    severity: "warning",
    ...
    howToFix:
      "Re-run the audit after the site's rate limit resets, or ask the site owner to allow the Sightline-Audit crawler.",
  },
  "server-error": {
    severity: "critical",
